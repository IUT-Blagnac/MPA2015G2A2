 ...
 *
 */
public class SauvegarderProjet {
	private File file;
	 ...
	 * @param pFOPTI
	 */
	public SauvegarderProjet(final FenetreOPTI pFOPTI) {
		...
		if (parcourir.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
			...
			String[] test = { "id", "groupe", "sujet", "client", "superviseur" };
			stringTemp.add(test.clone());
			for (i = 0; i < pFOPTI.getProjets().size(); i++) {
				test[0] = new Integer(pFOPTI.getProjets().get(i).getId())
						.toString();
				test[1] = pFOPTI.getProjets().get(i).getGroupe().getId();
				test[2] = "" + pFOPTI.getProjets().get(i).getSujet().getId();
				test[3] = "" + pFOPTI.getProjets().get(i).getClient().getId();
				test[4] = ""
						+ pFOPTI.getProjets().get(i).getSuperviseur().getId();
				stringTemp.add(test.clone());
			}
